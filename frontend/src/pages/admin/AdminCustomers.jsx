    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))", gap:10, marginBottom:22 }}>
        {[
          ...
          { label:"No Orders Yet",   value:customers.filter(c=>getCustomerOrders(c.email).length===0).length, color:"rgba(255,255,255,0.4)" },
      ...
      <div style={{ position:"relative", marginBottom:18 }}>
        ...
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search by name or email…"
          style={{ width:"100%", background:"rgba(255,255,255,0.05)", border:"1px solid rgba(255,255,255,0.1)", borderRadius:9, outline:"none", color:"#fff", fontSize:13, fontFamily:"'Inter',sans-serif", padding:"10px 12px 10px 36px", boxSizing:"border-box" }} />
      </div>
      ...
      {loading ? (
        ...
        <div style={{ padding:40, textAlign:"center", color:"rgba(255,255,255,0.25)", fontFamily:"'Oswald',sans-serif", fontSize:16 }}>No customers found</div>
      ) : (
        <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
          {filtered.map(c => {
            ...
            return (
              <div key={c._id} style={{ background:"rgba(255,255,255,0.03)", border:"1px solid rgba(255,255,255,0.08)", borderRadius:12, overflow:"hidden" }}>
                {/* Row */}
                <div style={{ display:"flex", alignItems:"center", gap:14, padding:"14px 18px", cursor:"pointer" }}
                  ...
                  <div style={{ display:"flex", gap:20, flexShrink:0, alignItems:"center" }}>
                    <div style={{ textAlign:"center" }}>
                      ...
                      <div style={{ color:"rgba(255,255,255,0.35)", fontSize:10, textTransform:"uppercase" }}>Orders</div>
                    </div>
                    <div style={{ textAlign:"center" }}>
                      <div style={{ color:"#e30613", fontSize:16, fontWeight:700, fontFamily:"'Oswald',sans-serif" }}>${revenue.toFixed(0)}</div>
                      <div style={{ color:"rgba(255,255,255,0.35)", fontSize:10, textTransform:"uppercase" }}>Spent</div>
                    </div>
                    <div style={{ color:"rgba(255,255,255,0.25)", fontSize:10, textAlign:"right" }}>
                      <div>Joined</div>
                ...
                {isOpen && (
                  <div style={{ padding:"0 18px 18px", borderTop:"1px solid rgba(255,255,255,0.06)" }}>
                    <div style={{ marginTop:14, fontFamily:"'Oswald',sans-serif", fontWeight:600, fontSize:12, color:"rgba(255,255,255,0.4)", letterSpacing:"0.08em", textTransform:"uppercase", marginBottom:10 }}>Order History</div>
                    {cOrders.length === 0 ? (
                      <div style={{ color:"rgba(255,255,255,0.25)", fontSize:13 }}>No orders yet</div>
                    ) : (
